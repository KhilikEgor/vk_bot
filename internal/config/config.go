...
import (
	"strings"
	logger2 "vk_bot/internal/logger"
...
func NewConf(configPath string) (*Configuration, error) {
	...
	if configPath != "" {
		logger2.Message{Message: "Чтение конфигурации"}.Info()
		viperConfig.SetConfigFile(configPath)

		err := viperConfig.ReadInConfig()
		if err != nil {
			logger2.Message{Message: "Ошибка чтения конфигурации", Err: err}.AddField("config-path", configPath).Error()
			return nil, err
		}
	}

	setDefaultInt(viperConfig, LogLevel, 4)
	logger2.SetLevel(logrus.Level(viperConfig.GetInt(LogLevel)))
